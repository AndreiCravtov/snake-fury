...
buildInitialBoard ::
  ...
  Point ->
  RenderState
buildInitialBoard b snake apple = updateRenderState empty delta
  where
    empty = RenderState {board = emptyGrid b, gameOver = False}
    delta = RenderBoard [(snake, SnakeHead), (apple, Apple)]
...
updateRenderState rs (RenderBoard delta) = rs {board = board rs // delta}
...
render b rs
  | gameOver rs = render b (rs {board = emptyGrid b, gameOver = False})
  | otherwise = foldl' (++) "" ([foldLine y | y <- [1 .. (height b)]])
  where
    foldLine y = line ++ "\n"
      where
        line = foldl' (++) "" [ppCell . (board rs !) $ (y, x) | x <- [1 .. (width b)]]
